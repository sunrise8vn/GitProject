...
function chatNO() {
	...
	if(nokia.getState() == true && txtNO.value.trim().length > 0){
		...
		nokia.setMess(count, nokia.name, iphone.name, txtNO.value, getToday());
		nokia.resetDraft();
		let text1 = "";
		...
		text1 += "</div>";
		text1 += "<p style='text-align: left;'>" + txtNO.value + "</p>";
...
function getInboxNO(idElement, deviceName, sender, receiver) {
	...
	this.m = nokia.getNewMess('nokia');

	if (m.length > 0) {
		for (let i = 0; i < m.length; i++) {
			let text1 = "";
			text1 += "<li class='right clearfix  w3-animate-bottom mess'>";
...
function getInboxIP(idElement, deviceName, receiver) {
	...
	this.newMess = iphone.getNewMess('iphone');
	...
	if (newMess.length > 0) {
		for (let i = 0; i < newMess.length; i++) {
			...
			text1 += "<li id='mess" + newMess[i].id + "' class='right clearfix  w3-animate-bottom mess'>";
			text1 += "<span class='chat-img pull-right'>";
			text1 += "<img src='http://placehold.it/50/FA6F57/fff&text=" + deviceName + "' alt='User Avatar' class='img-circle' />";
			text1 += "</span>";
			text1 += "<div class='chat-body clearfix'>";
			text1 += "<div class='divhode' style='width: 30px; line-height: 50px; float: left;'>";
			text1 += "<input type='checkbox' name='mess[]' value='"+ newMess[i].id +"'></div>"
			text1 += "<div class='header'>";
			text1 += "<small class='pull-left text-muted'><span class='glyphicon glyphicon-time'></span>" + newMess[i].date + "</small>"
			text1 += "<strong class='primary-font'>" + newMess[i].sender + "</strong>";
			text1 += "</div>";
			text1 += "<p style='text-align: left'>" + newMess[i].content + "</p>";
			element.append(text1);
			// element.prepend(text1);
		}
		iphone.clearNewMess();
...
function allMessageNO(idElement, deviceName, classElement, sender) {
	...
	this.m = nokia.getArchiveMess();

	for(let i = 0; i < m.length; i++){
		let text1 = "";
		if(this.m[i].sender == sender) {
			text1 += "<li class='left clearfix'>";
			text1 += "<span class='chat-img pull-left'>";
			text1 += "<img src='http://placehold.it/50/55C1E7/fff&text=" + deviceName + "' alt='User Avatar' class='img-circle' />";
			text1 += "</span>";
			text1 += "<div class='chat-body clearfix'>";
			text1 += "<div class='header'>";
			text1 += "<strong class='primary-font'>" + this.m[i].receiver + "</strong>";
			text1 += "<small class='pull-right text-muted'><span class='glyphicon glyphicon-time'></span>" + this.m[i].date + "</small>"
			text1 += "</div>";
			text1 += "<p>" + this.m[i].content + "</p>";
		}
		else {
			text1 += "<li class='right clearfix'>";
			text1 += "<span class='chat-img pull-right'>";
			...
			text1 += "<div class='chat-body clearfix'>";
			text1 += "<div class='header'>";
			text1 += "<small class='pull-left text-muted'><span class='glyphicon glyphicon-time'></span>" + this.m[i].date + "</small>"
			text1 += "<strong class='primary-font'>" + this.m[i].sender + "</strong>";
			text1 += "</div>";
			text1 += "<p style='text-align: left'>" + this.m[i].content + "</p>";
		}
		eName.append(text1);
	}
}
...
//     $("#home-call").on('click', function (e) {
//         (longpress) ?  e.preventDefault() : showComposeIP();
//     });
    
//     var startTime, endTime;
//     $("#home-call").on('mousedown', function () {
//         startTime = new Date().getTime();
//     });

//     $("#home-call").on('mouseup', function () {
...
function actionClickAllIP(elementId) {
    ...
    $(elementId).on('click', function (e) {
        (longpress) ?  e.preventDefault() : allMessage('#IPHONE', 'IP', 'all', 'iphone');
    });
...
function disableScreen(id, point, opac) {
	// $(id + " .home-menu").css("pointer-events", point);
	// $(id + " .home-menu").css("opacity", opac);
	// $(id + " .home-center").css("pointer-events", point);
	// $(id + " .home-center").css("opacity", opac);
...
function selectAll() {
    var checkboxes = document.getElementsByName('mess[]');
