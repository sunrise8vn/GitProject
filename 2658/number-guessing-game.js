function playGame() {
	...
	while (counts < 3 && !success) {
		if (guessUser > random) {
			var conf = confirm("Bạn đoán lớn hơn so với máy tính, bạn có muốn chọn lại " + random);
			if (conf) {
				do {
					guessUser = parseInt(prompt('Nhập giá trị máy tính sẽ chọn trong khoảng trên'));
				}
		...
		if (guessUser < random){
			var conf = confirm("Bạn đoán nhỏ hơn so với máy tính, bạn có muốn chọn lại " + random);
			if (conf) {
				do {
					guessUser = parseInt(prompt('Nhập giá trị máy tính sẽ chọn trong khoảng trên'));
				}
